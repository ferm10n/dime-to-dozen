...
import { budgets, expenses, groups } from './schema.ts';
...
async function seed() {
  ...
  for (const month of months) {
    for (const group of groupNames) {
      ...
      await db.insert(budgets).values({ group, month, amount })
        .onConflictDoNothing();
    }
  }
  ...
  await db.insert(groups).values({ group: 'NoBudgetGroup' })
    .onConflictDoNothing();
  await db.insert(expenses).values({
  ...
  await db.insert(groups).values({ group: 'NoExpenseGroup' })
    .onConflictDoNothing();
  ...
  await db.insert(groups).values({ group: 'OneMonthGroup' })
    .onConflictDoNothing();
  await db.insert(budgets).values({
    group: 'OneMonthGroup',
    month: prevMonth,
